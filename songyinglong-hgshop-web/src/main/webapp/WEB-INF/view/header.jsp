...
<div class="container">
		<div class="row">
			...
			<div class="col-md-6">
				<form action="/list" method="post" id="serchForm">
					<div>
					...
					<div id="cat" class="cat-dropdown">
						<div class="sidebar-contant">
											<div id="menu" class="navbar-collapse collapse">
												...
												<li class="level">
													<a href="/list?categoryId=${category.id }" class="page-scroll"> <i class="fa fa-desktop"></i>${category.name }</a>
															...
															<div class="megamenu-inner-top">
																<ul class="sub-menu-level1">
																	<c:forEach items="${category.childs}" var="secondCategory">
																		<li class="level2">
																			<a href="/list?categoryId=${secondCategory.id }">
																				<span>${secondCategory.name }</span>
																					</a>
																			<ul class="sub-menu-level2 ">
																				<c:forEach items="${secondCategory.childs}" var="threeCategory">
																					<li class="level3"><a href="/list?categoryId=${threeCategory.id }"><span>■</span>${threeCategory.name }</a></li>
																				</c:forEach>
																			</ul>
																		</li>
																	</c:forEach>
...
--%>
					<c:forEach items="${navCategories}" var="category">
						<li><a href="/list?categoryId=${category.id }">${category.name }</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
