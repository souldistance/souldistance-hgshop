...
public class SpuServiceImpl implements SpuService{
    ...
    public PageInfo<Spu> spuList(Spu spu, Integer pageNum, Integer pageSize) {
        ...
        if(spu.getGoodsName()!=null && !"".equals(spu.getGoodsName())){
            criteria.andGoodsNameLike("%"+spu.getGoodsName()+"%");
        }
        if(spu.getCaption()!=null && !"".equals(spu.getCaption())){
            criteria.andCaptionLike("%"+spu.getCaption()+"%");
        }
        ...
        return new PageInfo<Spu>(spus,5);
    }
     ...
     */
    @Override
    public void deleteSpuByIds(Integer[] ids) {
        for (Integer id : ids) {
     ...
     */
    @Override
    public void saveOrUpdateSpu(Spu spu) throws Exception {
        if (spu.getId() == null) {
            //新增
            ...
            Spu s = spuMapper.selectByPrimaryKey(spu.getId());
            if(i>0 && ("0").equals(s.getIsMarketable())){
                saveOrUpdateESSpu(s.getId());
     ...
     */
    @Override
    public List<Spu> selectSpuAll() {
        return spuMapper.selectSpuAll();
    }
     ...
     */
    @Override
    public Map<String, Object> getSpuList(SpuVO spuVO, Integer pageNum, Integer pageSize) {
        Map<String, Object> map = new HashMap<>();
        //①查询spuId列表
        List<Integer> spuIds=new ArrayList<>();
        if(spuVO.getCategoryId()!=null){
            List<Integer> childIds=categoryMapper.getChildsIdByCategoryId(spuVO.getCategoryId());
            if(childIds.size()>0){
                for (Integer childId : childIds) {
                    List<Integer> childIds2 = categoryMapper.getChildsIdByCategoryId(childId);
                    if(childIds2.size()>0){
                        for (Integer cid : childIds2) {
                            spuVO.setCategoryId(cid);
                            spuIds.addAll(spuMapper.selectSpuIdsBySearch(spuVO));
                        }
                    }else{
                        spuVO.setCategoryId(childId);
                        spuIds.addAll(spuMapper.selectSpuIdsBySearch(spuVO));
                    }
                }
            }else{
                spuIds = spuMapper.selectSpuIdsBySearch(spuVO);
            }
        }else{
            spuIds = spuMapper.selectSpuIdsBySearch(spuVO);
        }
    ...
    @Override
    public void spuMarketable(Spu spu) throws Exception {
        int i = spuMapper.updateByPrimaryKeySelective(spu);
        if(i>0 && ("0").equals(spu.getIsMarketable())){
            saveOrUpdateESSpu(spu.getId());
        }
        if(("1").equals(spu.getIsMarketable())){
            elasticsearchTemplate.delete(ESSpu.class,spu.getId().toString());
        }
    }
}
